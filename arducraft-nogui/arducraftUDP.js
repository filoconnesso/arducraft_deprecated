...
/*
 * FILO CONNESSO ARDUCRAFT LIBRARY DEAMON
 *
 *
...
async function Init(Arguments) {
  ...
  if (Arguments.length >= 4) {
    configData.ip_address = Arguments[0];
    configData.board_port = Arguments[1];
    configData.hostname = Arguments[2];
    configData.port = Arguments[3];
    configData.version = Arguments[4];
    configData.bot_name = Arguments[5];
  ...
  function StartBot(ip_address, board_port, hostname, port, version, bot_name) {
    board_port_addr = board_port;
    board_ip = ip_address;
